...
func ReadHeader(r io.Reader) (*Header, error) {
	...
	fmt.Printf("[ReadHeader] after Read, buf: %v\n", buf)
	...
	fmt.Printf("[ReadHeader] sizeOfL  buf: %v\n", buf)
	...
	fmt.Printf("[ReadHeader] header.Cookie: %v\n", header.Cookie)
	...
	fmt.Printf("[ReadHeader] after read header.Cookie: %v\n", header.Cookie)
...
func ReadPacket(r io.Reader, expectedCookie []byte) ([]Record, error) {
	...
	fmt.Printf("expected: %v, header: %v \n", expectedCookie, header.Cookie)
	if expectedCookie != nil && bytes.Compare(expectedCookie, header.Cookie) != 0 {
		s := fmt.Sprintf("expected cookie did not match, expected: %v, header: %v\n", expectedCookie, header.Cookie)
...
func WritePacket(w io.Writer, values ...interface{}) ([]byte, error) {
	...
	fmt.Printf("cookie: %v, cookieBytes: %v, header: %v, hdrBytes: %v\n", cookie, cookieBytes, header, hdrBytes)
